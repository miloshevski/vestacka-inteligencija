...
def removed_col(train_X, col):
    return [[row[i] for i in range(len(row)) if i != col] for row in train_X]
...
if __name__ == '__main__':
    ...
    if accuracy_train > accuracy_val * 1.15:
        train_X_new = removed_col(train_X, index_to_remove)
        scaler.fit(train_X_new)
        classifier.fit(scaler.transform(train_X_new), train_Y)
        to_classify = [to_classify[i] for i in range(len(to_classify)) if i != index_to_remove]
