...
if __name__ == '__main__':
    ...
    neuron = MLPClassifier(hidden_layer_sizes=(10,), activation='relu',learning_rate_init=0.001,random_state=0)

    class_0 = [row for row in dataset if row[-1] == 0]
    class_1 = [row for row in dataset if row[-1] == 1]

    train_set = class_0[:int(X*len(class_0))] + class_1[:int(X*len(class_1))]
    test_set = class_0[int(X*len(class_0)):] + class_1[int(X*len(class_1)):]

    train_X, train_Y = split(train_set)
    test_X, test_Y = split(test_set)

    models = [bayes,tree,forest,neuron]
    ...
    index = names.index(max_name)

    tp,fn=0,0
    for row_x, class_ in zip(test_X, test_Y):
        votes = [0, 0]
        for i, model in enumerate(models):
            weight = 2 if i == index else 1
        ...
        if final == 1 and final == class_:
            tp += 1
        elif final == 0 and final != class_:
            fn += 1
